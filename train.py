    ...
    def run_crossval(self):
        ...
        for fid in range(self.opt.crossval):
            ...
            val_data_loader = DataLoader(
                dataset=valset, batch_size=self.opt.batch_size, shuffle=False
            )

            self._reset_params()
            best_model_path, best_model_filename = self._train(
                criterion, optimizer, train_data_loader, val_data_loader, fid
            )
            ...
            self.evaluator.print_stats(
                test_stats, "evaluation of fold {} on test-set".format(fid)
            )

        logger.info(
            "aggregating performance statistics from all {} folds".format(
                self.opt.crossval
            )
        )
...
def prepare_and_start_instructur(opt):
    ...
    if opt.spc_lm_representation_distilbert:
        logger.info(
            "spc_lm_representation_distilbert defined, overwriting spc_lm_representation"
        )
    ...
    input_columns = {
        "global_lcf": [
            "special_text_target",
            "segments_ids_text_target",
            "text_raw_with_special_indices",
    ...
    if not opt.pretrained_model_name or opt.pretrained_model_name == "default":
        if opt.model_name in ["ram", "aen_glove"]:
            ...
            opt.pretrained_model_name = model_name_to_pretrained_model_name[
                opt.model_name
            ]
    else:
        path = os.path.join(
    ...
    if opt.device.type == "cuda":
        logger.info(
            "using GPU (cuda memory allocated: {})".format(
...
def parse_arguments(override_args=False):
    ...
    parser.add_argument(
        "--seed", default=1337, type=int, help="set seed for reproducibility"
    )
    parser.add_argument(
        "--local_context_focus",
        default="cdm",
    ...
    parser.add_argument(
        "--finetune_glove", type=str2bool, nargs="?", const=True, default=False
    )
    parser.add_argument(
    ...
    parser.add_argument("--focus_mode", type=str, default=None)
